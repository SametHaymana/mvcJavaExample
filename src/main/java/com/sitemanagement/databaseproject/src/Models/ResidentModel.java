...
public class ResidentModel  implements ModelInterface{
    // change database and table name
    private static final String TableName = "Project.Resident";
    ...
    public int insert(String fieldNames, List<Object> rows) throws Exception
    {
		...
		if (rows.get(i) instanceof Managers) {
			...
			Resident rst = (Resident)rows.get(i); 
	
			sql.append("(");
			for (int j=0; j<fieldList.length; j++) {
				String fieldName = fieldList[j].trim();
				sql.append(DatabaseUtilities.formatField(rst.getByName(fieldName)));
