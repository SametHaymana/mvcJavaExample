 ...
 * @author tomahawk
 */
public class OutcomeModel  implements ModelInterface{
    private static final String TableName = "Project.Outcome";
    ...
    public int insert(String fieldNames, List<Object> rows) throws Exception
    {
		...
		if (rows.get(i) instanceof Managers) {
			...
			Outcome out = (Outcome)rows.get(i); 
	
			sql.append("(");
			for (int j=0; j<fieldList.length; j++) {
				String fieldName = fieldList[j].trim();
				sql.append(DatabaseUtilities.formatField(out.getByName(fieldName)));
